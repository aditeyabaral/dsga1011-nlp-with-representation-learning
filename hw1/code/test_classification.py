...
def test_unigram():
    ...
    train_err = evaluate_predictor(
        [(ex, ex["gold_label"]) for ex in train_data], predictor
    )
    valid_err = evaluate_predictor(
        [(ex, ex["gold_label"]) for ex in valid_data], predictor
    )
    print("train error={}, valid error={}".format(train_err, valid_err))
    error_analysis(
        valid_data[:100], feature_extractor, weights, "error_analysis_unigram.txt"
    )
...
def test_custom():
    ...
    print("train error={}, valid error={}".format(train_err, valid_err))
    error_analysis(
        valid_data[:100], feature_extractor, weights, "error_analysis_custom.txt"
    )


test_unigram()  # train error=0.251, valid error=0.36
test_custom()  # train error=0.0152, valid error=0.3404

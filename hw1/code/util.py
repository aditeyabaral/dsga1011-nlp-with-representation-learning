...
def predict(weights, feat):
    ...
    return 1.0 / (1 + math.exp(-dot(weights, feat)))
...
def verbose_predict(feat, weights, label, fout):
    ...
    fout.write(
        "label={}, predicted={}, {}\n".format(
            label, y_hat, "correct" if correct else "wrong"
        )
    )
    for f, v in sorted(
        list(feat.items()), key=lambda x: x[1] * weights.get(x[0], 0), reverse=True
    ):
        w = weights.get(f, 0)
...
def error_analysis(examples, feature_extractor, weights, out):
    ...
    with open(out, "w") as fout:
        for ex in examples:
            ...
            fout.write(
                "P: {}\nH: {}\n".format(
                    " ".join(ex["sentence1"]), " ".join(ex["sentence2"])
                )
            )
...
def evaluate_predictor(examples, predictor):
    ...
    """
    return sum([predictor(feat) != label for feat, label in examples]) / float(
        len(examples)
    )

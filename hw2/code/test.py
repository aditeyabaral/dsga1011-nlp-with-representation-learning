...
def test_attention():
    ...
    assert torch.allclose(sum_attn, torch.ones_like(sum_attn).type_as(sum_attn)), print(
        "Incorrect attention weights", sum_attn
    )
    ...
    assert torch.allclose(attn[:, :, -1], torch.zeros_like(attn[:, :, -1])), print(
        "Attention weights are incorrectly masked"
    )
...
def test_positional_encoding():
    ...
    assert torch.allclose(
        pos_1, position.pe[1, :], atol=1e-02, rtol=1e-02
    ) and torch.allclose(pos_2, position.pe[2, :], atol=1e-02, rtol=1e-02), print(
...
def test_beam_search():
    ...
    assert match, print(
        "Beam search output with beam_size 1 should match greedy decoding"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--attention", action="store_true", help="unit tests for attention"
    )
    parser.add_argument(
        "--multiheaded_attention",
        action="store_true",
        help="unit test for mulitheaded attn",
    )
    parser.add_argument(
        "--positional_encoding",
        action="store_true",
        help="unit test for positional encoding",
    )
    parser.add_argument(
        "--beam_search", action="store_true", help="unit tests for beam search"
    )

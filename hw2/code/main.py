...
def create_dataloaders(
    device, vocab_src, vocab_tgt, spacy_de, spacy_en, batch_size=12000, max_padding=128
):
...
def train_worker(gpu, vocab_src, vocab_tgt, spacy_de, spacy_en, args):
    ...
    lr_scheduler = LambdaLR(
        optimizer=optimizer,
        lr_lambda=lambda step: rate(step, d_model, factor=1, warmup=args.warmup),
    )
    train_state = TrainState()
...
def run_model_eval(beam_search=False, beam_size=4, verbose=True, num_ex=None):
    ...
    _, valid_dataloader = create_dataloaders(
        0, vocab_src, vocab_tgt, spacy_de, spacy_en, batch_size=1
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--max_padding", type=int, default=72, help="Maximum sequence length"
    )
    parser.add_argument(
        "--warmup", type=int, default=3000, help="Number of warmup steps"
    )
    parser.add_argument(
        "--file_prefix",
        type=str,
        default="multi30k_model_",
        help="file prefix to use for saving",
    )
    parser.add_argument(
        "--beam_search",
        action="store_true",
        help="Use beam search decoding instead of greedy decoding",
    )
    parser.add_argument(
        "--beam_size", type=int, default=4, help="Beam size during beam search decoding"
    )

...
def call_together_api(prompt, student_configs, post_processing):
    output = together.Complete.create(
        prompt=prompt, model="meta-llama/Llama-2-7b-chat-hf", **student_configs
    )
    print("*****prompt*****")
    print(prompt)
    print("*****result*****")
...
if __name__ == "__main__":
    ...
    if not (len(fixed_keys) == len(student_config)):
        raise ValueError(
            f"expected to see a config dict of size {len(fixed_keys)}, but found {len(student_config)}"
        )
    ...
    for trial in range(
        3
    ):  # do 3 trials because same data same prompt could yield different result
        seed = 0
        rng = np.random.default_rng(seed)
        res_large = test_range(
            your_prompt(),
            student_config,
            n_sample=23,
            rng=rng,
    ...
    print(
        f"average acc is {np.mean(accs)}, average mae is {np.mean(maes)}, prompt_length is {p_len}"
    )
